...
(function(window) {
    ...
    function PinchUtils() {}
    PinchUtils.isFirefox = ua.indexOf('firefox') >= 0;
    PinchUtils.isAndroid = (ua.indexOf("Android") >= 0);
    PinchUtils.androidVer = (PinchUtils.isAndroid) ? parseFloat(ua.slice(ua.indexOf("Android") + 8)) : null;

    PinchUtils.objectToString = function(o) {
    ...
    PinchUtils.getRealValue = function(str) {
        val = str;
        if (str !== undefined) {
            if (!isNaN(Number(str))) {
                val = Number(str);
                ...
                var temp = PinchUtils.getObj(str);
                if (temp != null) {
                    val = temp;
                }
            }
    ...
    PinchUtils.getObj = function(str) {
    ...
    PinchUtils.getObjects = function(value) {
        ...
        for (i = 0; i < length; i++) {
            var objectName = PinchUtils.hyphenToCamelCase($.trim(objectNames[i]));
            var object = PinchUtils.getObj(objectName);
            if (object != null) {
                if ($.isArray(object)) {
    ...
    PinchUtils.getScope = function(str) {
        var f = window;
        var m = str.split(".");
    ...
    PinchUtils.getParams = function(str) {
        var params = null;
        if (str !== undefined) {
            params = [];
            ...
            for (i = 0; i < values.length; i++) {
                params.push(PinchUtils.getRealValue($.trim(values[i])))
            }
        }
    ...
    PinchUtils.trimSpaces = function(str) {
        var trimStr = "";
        if (str !== undefined) {
            trimStr = str;
    ...
    PinchUtils.getAttrObjectFromString = function(str, initObject) {
        str = str || "";
        ...
        if (str != "") {
            ...
            for (i = 0; i < length; i++) {
                var attr = groups[i].split(":"),
                    prop = PinchUtils.hyphenToCamelCase($.trim(attr[0])),
                    val = attr[1];
                ...
                val = PinchUtils.getRealValue($.trim(val));
    ...
    PinchUtils.getChildAttrObjectFromElem = function(elem, pluginName) {
        var o = {};
    ...
    PinchUtils.hyphenToCamelCase = function(value) {
        return value.replace(/-([a-z])/gi, function(s, g) {
            return g.toUpperCase();
        });
    }

    PinchUtils.getStyleSheetObject = function(selector) {
        var styleObj = {};
                ...
                if (selectorText !== undefined) {
                    ...
                    for (var k = 0; k < selectors.length; k++) {
                        ...
                        if (styleSelector == selector) {
                            styleObj = $.extend(styleObj, PinchUtils.getAttrObjectFromString(cssRules[j].style.cssText));
                        }
                    }
                }
            }
    ...
    PinchUtils.preventDefault = function(e) {
        if (e.preventDefault) {
            e.preventDefault();
        } else {
            e.returnValue = false;
    ...
    PinchUtils.preventGestureDefault = function(e) {
        e.gesture.preventDefault();
    }

    PinchUtils.shuffleArray = function(array) {
...
(function(window) {
    function PinchZoomer(_pinchZoomerDiv, imageUrl, varsObj) {
        ...
        function _init() {
            ...
            if (_vars.allowZoom) {
                imageHolder.on("mousewheel", PinchUtils.preventDefault);
            ...
            imageTouchHandler.on("drag", PinchUtils.preventGestureDefault);
            ...
            if (!isNaN(_vars.resizeDuration) && _vars.resizeDuration > 0) {
                TweenLite.to(this, _vars.resizeDuration, {
        ...
        function onImageLoad(e) {
            ...
            TweenLite.to(_image, _vars.animDuration, {
                autoAlpha: 1,
                ease: _vars.ease
            });
            TweenLite.set(_image, {
        ...
        function resetImage() {
            if (_imageLoaded) {
                ...
                var cssObj = {
                    width: _newImageWidth,
                    height: _newImageHeight,
                    left: realLeft + "px",
                    top: realTop + "px",
                    scale: 1,
                    transformOrigin: "0 0"
        ...
        function handleZoom(duration) {
            ...
            if (PinchUtils.androidVer !== null && PinchUtils.androidVer < 3) {
            ...
            TweenLite.to(_image, duration, cssObj);
        ...
        function setMove() {
            ...
            TweenLite.to(_image, _vars.animDuration, cssObj);
    ...
    PinchZoomer.init = function(imgs, _vars) {
        ...
        for (var i = 0; i < pinchZoomerLen; i++) {
            ...
            pinchZoomerObj._vars = $.extend({
                scaleMode: "widthOnly"
            }, (_vars.imageOptions || PinchUtils.getAttrObjectFromString(pinchZoomerObj.image.data("options"))));
            pinchZoomerObj.controlVars = $.extend({
                alwaysShow: false
            }, (_vars.controlOptions || PinchUtils.getAttrObjectFromString(pinchZoomerObj.image.data("control-options"))));
            ...
            pinchZoomerObj.zoomInButton.hammer().on("tap", {
                index: index
            }, onZoomIn);
            pinchZoomerObj.zoomOutButton.hammer().on("tap", {
                index: index
            }, onZoomOut);
            ...
            pinchZoomerObj.outerDiv.hammer().on("tap", {
                index: index
            }, onInput);
            pinchZoomerObj.outerDiv.on("mouseover", {
                index: index
            }, onInput);
            pinchZoomerObj.outerDiv.on("mouseout", {
                index: -1
            }, onInput);
        ...
        function getStyle(value) {
            ...
            if (value.indexOf(":") == -1) {
                ...
                style = PinchUtils.getAttrObjectFromString(value);
            }
        ...
        function resizeOuterDiv(index) {
            ...
            if (!pinchZoomerObj.removed) {
                ...
                if (pinchZoomer.imageLoaded()) {
                    ...
                    TweenLite.set(pinchZoomerObj.outerDiv, {
                        width: newOuterWidth,
                ...
                if (!pinchZoomerObj.removed) {
                    ...
                    if (curZoom == pinchZoomer.vars.minZoom) {
                        if (pinchZoomerObj.oldZoomOutEnabled || forceCss) {
                            TweenLite.set(pinchZoomerObj.zoomOutButton, pinchZoomerObj.zoomOutOffObj);
                        }

                        pinchZoomerObj.oldZoomOutEnabled = false;
                    } else {
                        if (!pinchZoomerObj.oldZoomOutEnabled || forceCss) {
                            TweenLite.set(pinchZoomerObj.zoomOutButton, pinchZoomerObj.zoomOutOnObj);
                        }
                    ...
                    if (curZoom == pinchZoomer.vars.maxZoom) {
                        if (pinchZoomerObj.oldZoomInEnabled || forceCss) {
                            TweenLite.set(pinchZoomerObj.zoomInButton, pinchZoomerObj.zoomInOffObj);
                        }

                        pinchZoomerObj.oldZoomInEnabled = false;
                    } else {
                        if (!pinchZoomerObj.oldZoomInEnabled || forceCss) {
                            TweenLite.set(pinchZoomerObj.zoomInButton, pinchZoomerObj.zoomInOnObj);
                        }
                    ...
                    if (i == index) {
                        if (!pinchZoomerObj.controlVisible) {
                            if (!pinchZoomerObj.controlVars.alwaysShow) {
                                TweenLite.set([pinchZoomerObj.zoomInButton, pinchZoomerObj.zoomOutButton], {
                                    visibility: "visible"
                                });
                            }
                        }
